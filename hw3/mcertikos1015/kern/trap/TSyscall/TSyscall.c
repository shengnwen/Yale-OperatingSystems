...
void sys_spawn(void)
{
  // TODO
	...
	if (elf_id == 1)
	{
		//if (_binary___obj_user_pingpong_ping_start == 0)
		//{
		//	syscall_set_errno(E_INVAL_ADDR);
        	//syscall_set_retval1(NUM_IDS);
        	//return;
		//}
		child = proc_create(_binary___obj_user_pingpong_ping_start, quota);		
	}
	else if (elf_id == 2)
	{
		//if (_binary___obj_user_pingpong_pong_start == 0)
        //{
           // syscall_set_errno(E_INVAL_ADDR);
            //syscall_set_retval1(NUM_IDS);
            //return;
        //}
		child = proc_create(_binary___obj_user_pingpong_pong_start, quota);
	}
	else if (elf_id == 3)
	{
		//if (_binary___obj_user_pingpong_ding_start == 0)
        //{
           // syscall_set_errno(E_INVAL_ADDR);
           // syscall_set_retval1(NUM_IDS);
           // return;
       // }
		child = proc_create(_binary___obj_user_pingpong_ding_start, quota);
	}
	else	
	{
