...
{
  struct file** filePointer = tcb_get_openfiles(get_curid());
  int i;
...
void sys_read(tf_t *tf)
{
  ...
  filePointer = tcb_get_openfiles(get_curid());

  fd = syscall_get_arg2(tf);
  buf = (char *)syscall_get_arg3(tf);
  ...
  readByte = file_read(filePointer[fd], kernelBuffer, len);
  if(readByte < 0) {
	syscall_set_errno(tf, E_BADF);
	syscall_set_retval1(tf, -1);
	return ;
  ...
  readByte = pt_copyout((void *)kernelBuffer, get_curid(), buf, readByte);
  if(readByte < 0) {
	syscall_set_errno(tf, E_BADF);
	syscall_set_retval1(tf, -1);
	return ;
  ...
  syscall_set_retval1(tf, readByte);
  return ;
}
...
void sys_write(tf_t *tf)
{
  ...
  wroteByte = pt_copyin(get_curid(), buf, kernelBuffer, len);
  if(wroteByte < 0) {
	syscall_set_errno(tf, E_BADF);
	syscall_set_retval1(tf, -1);
	return ;
  ...
  wroteByte = file_write(filePointer[fd], kernelBuffer, wroteByte);
  if(wroteByte < 0) {
	syscall_set_errno(tf, E_BADF);
	syscall_set_retval1(tf, -1);
	return ;
  ...
  syscall_set_errno(tf, E_SUCC);
  syscall_set_retval1(tf, wroteByte);
  return ;
}
...
void sys_fstat(tf_t *tf)
{
  ...
  if(file_stat(filePointer[fd], st) == -1) {
	syscall_set_errno(tf, E_BADF);
	syscall_set_retval1(tf, -1);
	return ;
  }
...
void sys_link(tf_t *tf)
{
  ...
  lenOld = syscall_get_arg4(tf);
  lenNew = syscall_get_arg5(tf);
  pt_copyin(get_curid(), syscall_get_arg2(tf), old, lenOld + 1);
  pt_copyin(get_curid(), syscall_get_arg3(tf), new, lenNew + 1);
